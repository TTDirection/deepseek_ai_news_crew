...
class TextSegmentOptimizer:
    ...
    def detect_list_structure(self, text: str) -> List[str]:
        """
        检测列表结构并分割
        ...
        for marker in list_markers:
            list_items = re.findall(f'({marker}[^{marker}\n]+)', text)
            if list_items and len(list_items) >= 2:
                print(f"检测到标记列表结构，包含 {len(list_items)} 个项目")
                return list_items
    ...
    def segment_chinese_text_with_llm(self, text: str) -> List[str]:
        ...
        try:
            ...
            # 检查是否有列表结构
            list_items = self.detect_list_structure(text_to_process)
            if list_items:
            ...
            try:
                ...
                if isinstance(tokens, list) and all(isinstance(item, str) for item in tokens):
                    ...
                    if len(tokens) >= min(2, min_segments_needed) and all(len(token) > 0 for token in tokens):
                        ...
                        segments.extend(self.optimize_segments(tokens))
                        # 打印所有分词结果
                        print("\n--------- LLM分词最终结果 ---------")
            ...
            # 所有方法都失败，返回原始文本
            print("LLM分词提取失败，使用原始文本")
            if len(segments) == 0:  # 如果之前没有添加过任何片段（包括标题）
    ...
    def smart_split_text(self, text: str) -> List[str]:
        ...
        segments = self.segment_chinese_text_with_llm(text)
